...
CREATE OR REPLACE FUNCTION apollo.reject_competition_safety_fact_change()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE TRIGGER competition_safety_reports_immutable
    BEFORE UPDATE OR DELETE ON apollo.competition_safety_reports
    FOR EACH ROW
    EXECUTE FUNCTION apollo.reject_competition_safety_fact_change();

CREATE TRIGGER competition_safety_blocks_immutable
    BEFORE UPDATE OR DELETE ON apollo.competition_safety_blocks
    FOR EACH ROW
    EXECUTE FUNCTION apollo.reject_competition_safety_fact_change();

CREATE TRIGGER competition_reliability_events_immutable
    BEFORE UPDATE OR DELETE ON apollo.competition_reliability_events
    FOR EACH ROW
    EXECUTE FUNCTION apollo.reject_competition_safety_fact_change();

CREATE TRIGGER competition_safety_events_immutable
    BEFORE UPDATE OR DELETE ON apollo.competition_safety_events
    FOR EACH ROW
    EXECUTE FUNCTION apollo.reject_competition_safety_fact_change();
