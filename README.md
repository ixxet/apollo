...
> Phase 3B.9 adds a public-safe availability read for booking options so
> customers can see requestable and unavailable time hints before submitting a
> request, without widening into public self-edit/rebook, self-booking, quotes,
> payments, or deployment truth. Phase 3B.10 adds the narrow APOLLO schedule
> safety needed by bounded staff schedule controls: schedule reads now expose a
> manage hint header, and booking-linked reservations cannot be cancelled
...
> remains legacy. Phase 3B.15 adds ARES v2 as an internal explainable
> proposal/match-preview foundation over APOLLO queue intent and legacy rating
> facts only. Phase 3B.16 adds internal competition analytics stat events and
> projections over canonical result and legacy rating facts only, while
> Phase 3B.17 adds the internal staff-only tournament runtime: tournament,
...
> tournament, public, or game identity truth. Phase 3B.19 adds public-safe
> competition readiness contracts and leaderboard projections derived only from
> finalized/corrected canonical results plus the legacy active rating path.
> Private/internal safety, manager, command, OpenSkill comparison, ARES
> proposal, tournament ops, and analytics internals remain non-public. Phase
...
| Competition command execution | `POST /api/v1/competition/commands` and `apollo competition command run` | Real in repo/runtime | Shared command/outcome/dry-run contract over existing competition services; live commands require role capability plus trusted-surface proof, idempotency support is reported explicitly, result lifecycle commands support dry-run/apply with result-version guards, and the internal command path can update queue intent tier facts and generate ARES v2 match previews |
| Competition tournament runtime | `GET/POST /api/v1/competition/tournaments`, `GET /api/v1/competition/tournaments/{id}`, `POST /api/v1/competition/tournaments/{id}/{seed,teams/lock,matches/bind,rounds/advance}`, and `apollo competition tournament {list,show}` | Real in repo/runtime | Staff/internal tournament runtime only; reads require `competition_read`, writes require the matching competition capability plus trusted-surface proof, and round advancement consumes finalized/corrected canonical result truth without mutating result, rating, analytics, ARES, or public truth |
| Public competition readiness | `GET /api/v1/public/competition/readiness` | Real in repo/runtime | Unauthenticated public-safe contract read; reports contract/projection versions, available projection counts, finalized/corrected canonical result source, and legacy active rating source without exposing private safety, manager, command, OpenSkill comparison, ARES proposal, tournament ops, or analytics internals |
| Public competition leaderboard | `GET /api/v1/public/competition/leaderboards` | Real in repo/runtime | Unauthenticated public-safe leaderboard projection over APOLLO analytics projections; returns redacted participant labels and allowlisted sport/mode/facility/stat facts derived from finalized/corrected canonical results plus legacy active ratings, without user IDs, result IDs, OpenSkill comparison facts, safety/reliability facts, or internal watermarks |
| Public/member game identity | `GET /api/v1/public/competition/game-identity`, `GET /api/v1/competition/game-identity` | Real in repo/runtime | Additive Phase 3B.20 projection contracts for CP, badge award facts, rivalry state facts, and squad identity facts; derived from APOLLO public-safe competition projections only, redacted publicly and self-scoped for members, with explicit CP/badge/rivalry/squad policy versions and no OpenSkill comparison, safety/reliability, command, ARES proposal, tournament ops, source-result, or watermark exposure |
...
| Public booking request intake | `GET /api/v1/public/booking/options`, `GET /api/v1/public/booking/options/{optionID}/availability`, `POST /api/v1/public/booking/requests` | Real in repo/runtime | Unauthenticated bounded intake; options expose only active/bookable/public-labeled choices, availability returns public-safe requestable windows and unavailable blocks only, submit requires an idempotency key, creates only `requested` public-source requests, returns an opaque public receipt code, and never creates schedule blocks |
...
| competition session / team / roster / match containers, queue/assignment/lifecycle truth, result capture, ratings, standings, self-scoped member stats, public-safe competition readiness/leaderboard projections, public/member-safe game identity projections, internal tournament runtime facts, manager/internal safety/reliability facts, and the bounded competition staff authz substrate | public tournaments, role-management product flows, facility-scoped staffing, persistent approval objects, messaging/chat, public profiles, and broad public social competition surfaces |
...
| public competition projection contracts | Real in repo/runtime | Public-safe HTTP contracts project only allowlisted readiness and leaderboard facts from finalized/corrected canonical results plus legacy active rating fields; they redact participant identity and exclude OpenSkill comparison facts, safety/reliability facts, command/readiness internals, ARES proposal facts, tournament ops truth, source result IDs, and projection watermarks |
| game identity projection contracts | Real in repo/runtime | Public/member-safe HTTP contracts derive CP, badge award, rivalry state, and squad identity facts from public-safe competition projection rows only; public output uses redacted participant labels, member output is self-scoped, policy versions are explicit, and private/internal result IDs, OpenSkill comparison facts, safety/reliability facts, command/readiness internals, ARES proposal facts, tournament ops truth, analytics sample/confidence, and projection watermarks remain non-public |
...
| public intake/status shape | public options return active/bookable/public-labeled choices only; public availability returns requestable windows and generic closed/booked/unavailable blocks only; public submit accepts option ID, contact fields, organization, purpose, attendee count, and RFC3339 windows; the response returns a public receipt code and public status; lookup by receipt code returns public status, optional public message, requested window, and update time while omitting request UUIDs, conflicts, staff notes, actor/session IDs, trusted-surface data, schedule block IDs, graph internals, quotes, and payment fields |
...
| cancellation shape | cancelling an approved request reuses `POST /api/v1/booking/requests/{id}/cancel`, locks and validates the linked reservation, cancels it, and retains `schedule_block_id` for audit; generic schedule-block cancel refuses booking-linked reservations |
...
| deferred with this line | public self-booking, public self-edit/rebook, broader customer self-service/status portal, in-place approved editing, payments, quotes, invoices, deposits, owner policy writes, recurrence, broad operating-hours policy editing, admin role widening, gateway, HERMES, AI/LLM negotiation, and deploy work |

Phase 3B.10 bounded staff schedule-control support is now real in repo/runtime
on `main`, with deployed truth unchanged. APOLLO remains the schedule authority:
supervisors can read schedule blocks, managers and owners can create typed
...
Phase 3B.15 closes only the ARES v2 proposal/match-preview foundation:
queue intent facts, internal preview projections/events, APOLLO-computed
match quality, APOLLO-computed predicted win probability, and explicit
explanation codes are real. ARES remains a proposal engine and does not own
match lifecycle, canonical results, rating truth, booking/commercial truth, or
public competition truth. The active rating read path remains the legacy
projection.

Phase 3B.16 closes only the internal competition analytics foundation:
APOLLO stores derived stat events and projections for matches played,
wins/losses/draws, current streak, legacy rating movement, opponent strength,
team-vs-solo delta where participant-per-side facts support it, and
facility/sport/mode splits. Analytics consume finalized/corrected canonical
results plus legacy rating facts only, record explicit sample size/confidence,
computed time, projection version, and projection watermark, and do not mutate
...
binding, round advancement, explicit advance-reason, and tournament event
...
tournament, lifecycle, booking, public, member, or game identity truth.

Phase 3B.19 closes only public competition readiness: APOLLO exposes
public-safe readiness and leaderboard projection contracts derived from
finalized/corrected canonical result truth plus the legacy active rating path.
The public contracts do not expose private/internal safety, manager, command,
OpenSkill comparison, ARES proposal, tournament ops, source-result, projection
watermark, or operational truth.

Phase 3B.20 closes only the game identity layer: APOLLO exposes public and
member-safe game identity projections for CP, badge awards, rivalry state, and
squad identity. Those projections consume APOLLO public-safe competition
projection rows only and do not mutate canonical result, rating, analytics,
ARES, tournament, safety, public-readiness, booking, or member truth. Public
tournaments, messaging/chat, broad public social graph, public profiles,
...
| CLI | Cobra | Instituted | `v0.2.x` -> `v0.6.0` | `serve`, `visit list`, sport reads, schedule reads, and the service-backed `competition command` runner are real |
...
- `GET /api/v1/competition/commands/readiness`,
  `POST /api/v1/competition/commands`, and `apollo competition command run`
  are real as the shared command/readiness foundation over existing competition
  service behavior; dry-run plans do not mutate, live commands keep existing
  capability and trusted-surface checks, and result command apply remains
  deferred beyond dry-run planning
...
| `Phase 3B.6` | - | Runtime-local on `main` | APOLLO-owned public request intake API with public-safe options, idempotent neutral request submit, public source/channel truth, and no schedule-block creation until staff approval | instant booking, status portal, customer self-service, payments, quotes, invoices/deposits, Hestia booking UI, owner policy writes, admin role widening, HERMES/gateway/deploy work |
| `Phase 3B.7` | - | Runtime-local on `main` | APOLLO-owned public receipt/status lookup plus staff-authored public-safe customer messages separate from internal notes | instant booking, broader customer self-service/status portal, edit/rebook, payments, quotes, invoices/deposits, owner policy writes, admin role widening, AI/LLM negotiation, HERMES/gateway/deploy work |
...
| `Phase 3B.20` | - | Runtime-local on `main` | Game identity layer: public/member-safe CP, badge award, rivalry state, and squad identity projection contracts over public-safe competition projection rows | Messaging/chat, broad public social graph, public safety details, public tournaments, OpenSkill read-path switch, proposal workflows, SemVer governance, fake UI data, and deploy claims |
...
repo/runtime closeout truth on `main` includes Phase 3B.20 game identity over
Phase 3B.19 public competition readiness, Phase 3B.18 social
safety/reliability foundation, Phase 3B.17 internal tournament runtime, Phase
3B.16 competition analytics foundation, Phase 3B.13 legacy rating foundation,
Phase 3B.14 OpenSkill comparison evidence, Tracer 28 authz/staff-boundary
truth, and the Milestone 2.0 hardening follow-up closed on `v0.19.1`. Later
