...
type Store interface {
	...
	ArchiveSession(ctx context.Context, sessionID uuid.UUID, ownerUserID uuid.UUID, archivedAt time.Time) (sessionRecord, error)
...
func (s *Service) ArchiveSession(ctx context.Context, ownerUserID uuid.UUID, sessionID uuid.UUID) (Session, error) {
	...
	draftMatches, err := s.repository.CountDraftMatchesBySessionID(ctx, sessionID)
	if err != nil {
		return Session{}, err
	}
	if draftMatches > 0 {
	...
	archived, err := s.repository.ArchiveSession(ctx, sessionID, ownerUserID, s.now().UTC())
	if err != nil {
		return Session{}, err
	}
