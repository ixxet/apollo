...
func TestLobbyEligibilityEndpointIsSideEffectFreeAcrossVisitsWorkoutsAndClaimedTags(t *testing.T) {
	...
	if _, err := env.db.DB.Exec(context.Background(), "INSERT INTO apollo.workouts (user_id, started_at, status, finished_at) VALUES ($1, $2, $3, $4)", user.ID, time.Date(2026, 4, 2, 15, 5, 0, 0, time.UTC), "finished", time.Date(2026, 4, 2, 16, 5, 0, 0, time.UTC)); err != nil {
